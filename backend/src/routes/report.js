...
// POST /api/report - Tạo mới rescue request
router.post("/", async (req, res) => {
  try {
    ...
    // Tạo mã case dễ đọc: RES-YYYYMMDD-ID
    const now = new Date();
...
// GET /api/report/status/:caseCode - Tra cứu trạng thái case
router.get("/status/:caseCode", async (req, res) => {
  try {
    const { caseCode } = req.params;

    // Extract ID from caseCode (e.g. RES-20240515-0001 -> 1)
